...
export default class Grid {
  ...
  render(wStage, hStage) {
    ...
    this.text("[0,0]", center.shift(new Offset(3, -3).withMultiplier(this._scale)).shift(this._offset))
    ...
    for (var i = 1; i < vLeftGridTotal; i++) {
      ...
      this.text(50 * i, new Point(wStage / 2 + i * wGridStep, hStage / 2).shift(new Offset(3, 10).withMultiplier(this._scale)).shift(this._offset));
    }
    ...
    for (var i = 1; i < hTopGridTotal; i++) {
      ...
      this.text(-50 * i, new Point(wStage / 2, hStage / 2 + i * wGridStep).shift(new Offset(3, 10).withMultiplier(this._scale)).shift(this._offset));
    }
  ...
  text(t, s) {
    ...
    this.ctx.font = (this._scale * 10) + "px Tahoma";
